...
socket.on("submit-task", async (data) => {
  try {
    ...
    // Notify instructor live if inside a room
    if (activeRooms[roomId]) {
      const instructorSocket = activeRooms[roomId].adminSocket || activeRooms[roomId].assistantSocket;
      if (instructorSocket) {
    ...
    socket.emit("submission-saved", { message: "✅ Submission saved successfully" });
  } catch (err) {
    console.error("Task submission error:", err);
    socket.emit("submission-error", { error: "Server error saving submission" });
  }
});
